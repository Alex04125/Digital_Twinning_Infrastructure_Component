...
const CreateInstanceForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post("/api/create_instance", data);
      if (response.data.error) {
        setModalMessage(response.data.error);
      } else {
        setModalMessage(
          "Instance created successfully! You can check its status on the Instances page."
        );
      }
    ...
    <FormContainer>
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button type="submit" disabled={loading}>
          {loading ? <ClipLoader size={20} color="#ffffff" /> : "Create Instance"}
        </Button>
      </Form>
      <Modal
        isOpen={modalIsOpen}
        onRequestClose={closeModal}
        contentLabel="Instance Creation Status"
        ariaHideApp={false}
        style={{
          content: {
            top: "50%",
      ...
      >
        <ModalContent>
          {modalMessage && modalMessage.includes("Instance name already exists") ? (
            <>
              <h2>Error</h2>
              <p>{modalMessage}</p>
            </>
            ...
            <>
              ...
              <p>
                ...
                <a href="/instances">Instances</a> page.
              </p>
            </>
          )}
